...
**Status: optional reinforcement complete (2026-07-23, nine of nine lessons)** — from a
...
(`0035`–`0043`) plus seven reference docs; all nine lessons have now shipped, generated one at a
time by request rather than as a single batch, closing with `0043`'s capstone.
...
| `0039-css-layout-flow-flex-grid-and-overflow.html` | The CSS prerequisite Tutorial 11 had to half-teach mid-debug, taught properly here around one throughline — identify every size constraint and overflow boundary in an element's ancestor chain, then name which region and axis each one owns (revised by review from an original, inaccurate "exactly one ancestor owns height, exactly one descendant owns scrolling" framing) — using `AppShell.tsx`, `app/(public)/layout.tsx`, `UploadPanel.tsx`, `Table.tsx`, and `ValueSection.tsx` as five real, verified examples: normal flow and border-box sizing (`Card.tsx`); flex main/cross axis traced through `AppShell`'s nested `flex`/`flex-col` (`TopHeader`'s `shrink-0` vs. `main`'s `flex-1`); grid tracks and the `fr` unit (`ValueSection`'s `grid-cols-1 md:grid-cols-[1fr_2fr]`, explicitly as a target ratio over *available* space, not a content-independent guarantee, per MDN's own note); intrinsic minimum size via `UploadPanel`'s two real fixes (explicit `min-w-0` vs. `truncate`'s `overflow:hidden` establishing a scroll container and so zeroing the automatic minimum — narrowed from an earlier overbroad "any non-`visible` overflow" claim, since `overflow: clip` is a documented exception); and overflow ownership as a per-region, per-axis question rather than one global scrollbar — `AppShell`'s `h-screen overflow-hidden` shell containing **two** independent real scroll owners (`<main>`'s `overflow-y-auto` for workspace content *and* `SidebarNav.tsx`'s own `overflow-y-auto` for the nav list, previously missed entirely), `(public)/layout.tsx`'s `min-h-screen` page-level-scroll strategy, and `Table.tsx`'s two-layer nested `overflow-hidden`/`overflow-x-auto` split; plus a corrected distinction between `overflow: hidden` (still a scroll container — programmatic scroll and focus-triggered scroll-into-view both still work) and `overflow: clip` (genuinely unreachable), replacing an earlier claim that `overflow-hidden` left content with "no way to reach it." Mobile-first breakpoints reuse examples already shown (`AppShell`'s `hidden lg:block`, `ValueSection`'s `md:` prefix) rather than adding new ones. Deliberately stops short of narrating Phase 9.1's specific historical fix to this same shell, per the task file's own instruction — general mechanics only, softened during authoring from a first draft that had asserted an unverified specific claim about what Phase 9.1 fixed |
| `0040-accessible-components-as-contracts.html` | Accessibility as part of a component's public behavior, not a final styling pass, using seven real components with genuinely mixed real results — revised by review from an earlier draft that presented several real accessibility gaps as working patterns. Semantic HTML before ARIA (`TopHeader`'s real `<button>` toggle correctly built), contrasted with `VerticalBucketBarChart`'s bare `<div tabIndex={0} aria-label=...>` bar segment — corrected to state that a plain div's implicit `generic` role **prohibits** accessible naming per the ARIA spec, so this `aria-label` may simply be ignored, not a valid "manual" mechanism; `DonutBreakdownChart`'s circles (`role="img"` + `aria-label`, a genuinely valid nameable pattern) added as the real working contrast, alongside its legend rows creating a second, independently-focusable, unnamed stop for the same datum — a new section this lesson's lede had promised but an earlier draft never actually delivered. Accessible names: `UploadPanel`'s visible `label` prop (e.g. "Orders File") corrected to show it's disconnected from the real `<label>` element wrapping the file input, so multiple `UploadPanel`s on one page compute to the same accessible name ("Choose a file… Browse") — plus a second, previously unmentioned gap, no `focus-within:` styling anywhere, so the `sr-only` input has zero visible focus indicator. `TopHeader`'s icon-only button's `aria-label` remains the correct contrasting example. ARIA computed from state, narrowed from an overclaim ("every ARIA attribute") to "every *stateful* ARIA attribute," since `aria-hidden`/`aria-controls` are intentionally literal; `role="alert"` reworded from "only works because of conditional rendering" to the general rule (dynamic content change vs. content already present at load). Decorative-icon `aria-hidden`, narrowed from "every decorative icon" to name `MetricCard`'s real, unhidden, undecorated icon (Lucide's installed version adds no `aria-hidden` by default, verified directly against `node_modules`). Visible focus, now opening with WCAG's three-part objective test (identifiable, persistent, distinguishable) before any judgment call, applied to `UploadPanel`'s objectively-failing case and three different real `focus:outline-none` replacements across both charts. `StatusBadge`/`Badge` color-plus-text, now citing WCAG's Use of Color criterion directly. Quiz 1 and Quiz 2 rewritten (the first previously contradicted the lesson's own TopHeader excerpt; the second previously overclaimed a general rule from one specific case). Deliberately stops short of the mobile drawer's own focus-trap behavior, reserved for the deferred mobile-nav retrospective, per the task file's explicit instruction |
| `0041-route-groups-layout-composition-and-metadata.html` | The post-Tutorial-11 structure the portfolio landing page introduced: `(public)`/`(workspace)` route groups organize `app/`'s six real routes without adding a URL segment (verified against the installed Next.js docs' own "Route groups" section, plus the real folder→URL table for all six current routes — `(workspace)` holds five, `(public)` holds one); the trimmed root `app/layout.tsx` (`html`/`body`/font only, per its own in-file comment) versus the two thin nested layouts (`(public)/layout.tsx`'s header/footer, `(workspace)/layout.tsx`'s `AppShell`) that supply each group's real chrome, with `docs/architect/portfolio-landing-page/decisions.md` cited directly for the real reason the split exists (no operational sidebar on the portfolio landing page) and a precisely-scoped named consequence (`<Link>`/router navigation between `/` and `/dashboard` stays one client-side transition, since this app defines only one root layout — revised by review from an earlier, unqualified "navigating stays client-side" claim, and now also naming route groups' own multiple-root-layout/shared-layout-subset use cases so "a filing decision, nothing more" isn't an undersell); static metadata as a Server-Component-only export, verified against both the "Metadata and OG images" page and the `generateMetadata` reference's own explanation of why (metadata must resolve server-side before the initial HTML) — revised by review to distinguish this app's actual all-static, prerendered metadata (resolved at build time, verified via `grep` that no `generateMetadata` function exists anywhere in `app/`) from the docs' separate streaming-metadata behavior for a dynamic `generateMetadata` function, which this app doesn't use; three of the five workspace pages (`order-validation`, `inventory-allocation`, `payment-aging`) have a current `page.tsx` module that cannot export metadata, being `"use client"` — revised by review to state this as a fact about the module, not an inherent limit of the route, since a Server-Component page wrapping a Client child could still export real metadata — while `dashboard`/`reports` stay Server Components that simply choose not to, verified against the `generateMetadata` reference's real "Merging"/"Inheriting fields" example and confirmed against this repo's own prerendered build output (`grep`-diffing `.next/server/app/index.html` against `dashboard.html`'s `<title>` tags) that `/dashboard` inherits the root layout's title completely unchanged. Revised by review to stop presenting the shared workspace title as an intentional SEO choice — no architecture doc records that decision and this app has no `robots`/`noindex` mechanism either — reframed as current behavior and a real, low-risk improvement, citing WCAG's Page Titled criterion for why unique titles matter beyond SEO. Named explicitly as a non-boundary: route groups carry no authorization meaning, since this app has no login step at all. Exercise is a real, reversible `npm run build` + `grep` comparison against `reports.html`, not a code edit, now with an explicit caveat that `.next/server/app/` is a current build artifact, not a stable public API. Quiz answer choices rebalanced to within one word of each other per group after an initial uneven draft |
| `0042-frontend-verification-layers.html` | What this repo's four verification mechanisms — three repository scripts (`npm run typecheck`, `npm run lint`, `npm run build`) plus a manual, developer-run `curl`-based `/health` diagnostic from `README.md`, never conflated with an automated check — each actually prove, using real, currently-necessary examples: `Badge.tsx`'s `resolvedTone` nullable-index pattern for typecheck (its exact `TS2538` error independently reproduced against this repo's own installed compiler via `./node_modules/.bin/tsc`, not `npx`'s potentially-different resolution — revised by review); Lesson 38's own real `react-hooks/exhaustive-deps` warning for lint, now explicit that it's a *warning* and this repo's bare `eslint` script (no `--max-warnings`) exits `0` regardless, per ESLint's own documented exit-code rule — revised by review; Tutorial 10 Part 3's real `"Functions cannot be passed directly to Client Components"` error for `next build`, now qualified as a build-time serialization check (not a "compile-time" one) and paired with this repo's own freshly-verified `next build` output showing all six routes plus `/_not-found` as `○ (Static) prerendered as static content` — revised by review to stop overclaiming "every route can render" as a general truth. A precise, version-specific fact anchors the "what overlaps" question: this repo's installed Next.js 16 fails `next build` on TypeScript errors by default, so `build` and `typecheck` genuinely overlap here, but `next build` no longer runs ESLint at all as of Next.js 16 (`next lint` itself was removed) — verified against the installed TypeScript-config and ESLint-plugin reference docs. Both of Tutorial 11's real UI bugs are used as the concrete counter-example, with the pointer-events story rewritten by review into two clearly separated findings — the real user-facing defect (the `inset-0` overlay intercepting pointer events across its *entire* bounding box, discovered because Playwright's `locator.hover()` runs an actionability check that detects an intercepting element and names it) versus a wholly separate, test-script-only problem (`hover()`'s default bounding-box-center target landing in an annulus's empty hole, fixed by switching to `.focus()`) — where an earlier draft had incorrectly conflated the two into a single "automation caught what a human's mouse trajectory missed" narrative. The hydration section's "byte-level comparison" wording was replaced with React's own `hydrateRoot` framing ("expects the rendered content to be identical with the server-rendered content") — revised by review. Introduces the installed Next.js testing guide's Unit/Component/Integration/E2E/Snapshot definitions now as an exact verbatim quotation (revised by review from a paraphrased block that had been mislabeled as one), and maps this repo honestly onto it: no committed, repeatable frontend test suite exists in any of the five categories (verified: no `*.test.*`/`*.spec.*` file anywhere, no `test` script) — phrased, per review, to stay compatible with the separate fact that ad-hoc Playwright testing has genuinely happened here twice, just never retained as regression coverage. Closes with a seven-line decision ladder (revised by review to split the old single "process boundary" entry into "is the backend reachable" vs. "is its behavior/contract correct"), and an exercise that reproduces the Badge.tsx bug in an untouched scratch file, now run via the repo's own installed compiler rather than bare `npx` |
| `0043-track-5-guided-ui-trace.html` | Capstone — no new concepts, matching Track 4's own `0030` capstone shape (predict-then-`<details>`-reveal traces, no code boxes). Three real traces: (1) a `ValidationErrorRow` from `types/index.ts` through page state, two layers of `useMemo`, `DataTable<T>`'s generic resolution, the real composite `getRowKey` (Lesson 38's still-unfixed non-uniqueness finding), down to semantic markup and the still-open `aria-sort` gap (Lesson 40) — revised by review to state `ValidationErrorRow` as a declared static contract only, since `lib/api-client.ts`'s `postJSON<T>` parses the response with a bare `as T` type assertion, no runtime validation; to stop crediting Lesson 36's snapshot rule for the whole-object-vs-split-fields state shape (a separate source-of-truth decision); to say `errors`/`filteredErrors` are recomputed only on `Object.is`-inequality, not "fresh every render"; to attribute `DataTable<T>`'s generic inference to the combined `columns`/`data`/`getRowKey` props, not `columns` alone; and to correct the `setCurrentResult(result)` citation from line 154 (which only declares the state) to line 184 (the actual call); (2) **rewritten after a critical review finding**: the dashboard crosses the Server/Client boundary *twice*, not once — `app/(workspace)/layout.tsx`'s `AppShell` is itself a real Client Component (`"use client"`, real drawer/focus/`usePathname` state), with the Server-rendered `DashboardPage` passed through as its `children` per the installed Next.js docs' own "Interleaving Server and Client Components" pattern (newly cited, real link), and `DashboardLiveSections` is a *second*, independent, more deeply-nested Client boundary — the original draft had wrongly claimed `DashboardLiveSections` was "the entire" boundary the page needed; also revised to separate independent per-workflow state (a modeling decision) from the snapshot rule itself, to soften the "only a real browser could catch this" claim into "choose the smallest layer that can observe the regression," and to name a small, real, verified gap (`lib/session-id.ts`'s stale "always returns a non-empty string" docstring, contradicted by its own `string | null` signature); (3) `/` and `/dashboard` side by side — revised to describe the root layout's and `(public)/layout.tsx`'s full real responsibilities (global stylesheet/font/body classes/fallback metadata; a real flex wrapper plus semantic `<main>`) rather than "html/body/font and nothing else" / "header/footer only," to qualify client-side navigation as specifically `<Link>`/router transitions, and to state that `AppShell`'s own sidebar/drawer/`TopHeader` are genuinely interactive chrome in their own right, not crediting `DashboardLiveSections` as Path B's only interactive part. Retrieval question 2 rewritten from a product-rationale question to the actual code mechanism (`status === "loading"`'s early return); all three quiz groups remain exactly 9 words per option. Interview drill restructured so the five-point rubric, a model answer, and a self-scoring guide sit behind a `<details>` reveal opened only after answering, rather than being shown upfront alongside the question. Now cites one reused primary source (the Next.js interleaving page) and links its principal source files, Tutorials 10/11, and every Lesson 35–42 reference in the closing paragraph, rather than leaving them as plain non-linking text |

New reference doc shipped alongside `0035`: `typed-react-component-contract.html`. `0036`–`0043`
shipped without their own reference docs — the task file's `react-render-effect-decision-guide.html`
reads as a synthesis of `0036`'s render/event material and `0037`'s Effect material specifically,
`list-identity-and-memoization-checklist.html` is `0038`'s intended companion,
`css-layout-debugging-checklist.html` is `0039`'s, `accessible-component-contract.html` is `0040`'s,
`app-router-layout-map.html` is `0041`'s, and `frontend-verification-ladder.html` is `0042`'s, all
per the task file's own reference-doc list; `0043` (the capstone) was never assigned a reference doc
by the task file at all — none requested this session.
...
link as soon as the next lesson actually ships — `0035`→`0036`, `0036`→`0037`, `0037`→`0038`,
`0038`→`0039`, `0039`→`0040`, `0040`→`0041`, `0041`→`0042`, and now `0042`→`0043`. `0043` is the
capstone and final lesson in this optional sequence — matching `0030`'s precedent exactly, both its
top and bottom nav read "Track complete" rather than pointing to a next lesson, since there isn't
one.
