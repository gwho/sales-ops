...
**Phase:** Phase 10 - FastAPI Integration (complete)  
**Last completed:** Phase 10 — a `backend/` FastAPI package (thin wrapper over the tested `src/` modules, per `docs/adr/0006-stateless-fastapi-workflow-and-report-exports.md`) plus live-wiring of all three workflow pages. `POST /api/orders/validate(/report)`, `POST /api/inventory/allocate(/report)` (runs `validate_orders` internally before `allocate_inventory`), `POST /api/payments/aging(/report)` (required client-sent `as_of_date`), and `GET /api/templates/{name}` (allowlisted `sample_data/*.xlsx` downloads) — no `GET /api/reports/{report_id}`, no persisted Workflow Run, no stored report artifacts; report endpoints re-accept raw files and recompute rather than trusting a client-supplied result. Uniform `{"detail": "string"}` error contract (`backend/errors.py` normalizes FastAPI's own list-shaped validation errors); known input/business failures → 400, unexpected → generic 500, `src/` itself stays framework-free (`backend/uploads.py` is the sole conversion boundary). Frontend: new `lib/api-client.ts` (low-level fetch/error/blob-download mechanics only, not a shared hook — each page owns page-local `RequestStatus`/`currentResult`/`errorDetail` state), `UploadPanel` gained `onFileChange`/`sampleFileName` props, all 3 workflow pages replaced their static `lib/mock-data.ts` reads with live requests plus an explicit "Run" action and a mid-session-added "Run sample data" shortcut (fetches the allowlisted sample files and runs the workflow on them, reusing the same submission path as a manual upload). Payment Aging's As-of Date is now enabled/required/editable, defaulting to today. `/dashboard` and `/reports` deliberately stay static/mock-derived (no cross-page store exists to feed them live data); `/reports` was reframed as a "Sample Report Overview" with cards linking to the corresponding workflow page instead of a disabled download button. `context/architecture.md`'s stale `GET /api/reports/{report_id}` endpoint list corrected to match. `uv run pytest` 194 passing (up from 172; 22 new `tests/test_backend_*.py` using FastAPI's `TestClient` against real `sample_data/*.xlsx`). `npm run typecheck`/`lint`/`build` all clean. Live-browser verification (Playwright, both dev servers running together) confirmed real computed results (36/28 orders, 28 allocation lines), a working report download, the enabled date field, a correct business-readable error render for a malformed upload, no CORS errors, and `/dashboard`/`/reports` unchanged visually.
**Next:** Undetermined — Phase 10 was the last phase defined in `context/build-plan.md`. Any further work (auth, persistence, deployment, additional workflows) is new scope requiring its own planning pass, not an implied next phase.
