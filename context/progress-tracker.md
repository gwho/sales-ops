...
**Phase:** Phase 9.1 - Visual Alignment Fixes (complete)  
**Last completed:** Phase 9.1 — a visual alignment review of Phase 9 (code + rendered-HTML audit, cross-checked against the 4 original Figma Make prototypes via Figma MCP) found 1 Critical layout bug and several Important/Minor issues, all fixed and manually screenshot-verified (before/after) this phase. Fixed: `AppShell`/`SidebarNav` height model (sidebar now genuinely spans full page height on content-heavy routes), sidebar/header brand-name mismatch, `/reports` container width, `WorkflowStepper`'s terminal-step color. Added: download actions on all 3 workflow pages, a static "As of" date on Payment Aging. Adopted 2 pre-approved-but-unbuilt Figma patterns: icon-chip `MetricCard`s and zebra-striped `DataTable` rows. 4 follow-on addenda (all built this phase, same session): table density/typography/truncation/sort-coverage pass across all 3 workflow pages; `FilterToolbar`/`FilterSelect`/`MiniBar` — client-side filtering and sorting added back to `DataTable` (revising Phase 9's "sort only" decision) plus per-column numeric alignment, long-text truncation+tooltip, and a `TableSectionHeading` icon+caption pattern; `/dashboard` gained "Inventory Shortage Alerts," "Payment Follow-up Items," a workflow infographic, and an English-only "How This Demo Works" guide; `DonutBreakdownChart`/`VerticalBucketBarChart` (new `components/charts/` folder, pure CSS/SVG, no library) replaced the dashboard's Allocation/Payment-Aging mini-bar visuals. `npm run build`/`lint`/`typecheck` clean throughout; `uv run pytest` still 152 (Python untouched); token/raw-color and Figma-reject-vocabulary scans clean at every checkpoint.
**Next:** Phase 10 (FastAPI Integration). Wires the 4 upload panels and report-download buttons to real endpoints, replaces `lib/mock-data.ts` static imports with live API calls, and gives `LoadingState`/`BusinessErrorMessage`/`ReportCard`'s non-Ready states and the report-lifecycle visual variants real client-driven transitions for the first time (all built as visual variants in Phase 9/9.1 but never live-wired). Bilingual (Traditional Chinese) guide content and a possible header-level per-column filter mechanism were both explicitly deferred in Phase 9.1 — revisit only if requested.
